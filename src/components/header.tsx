  ...
  return (
    <header className="fixed top-0 z-50 w-full border-b border-zinc-800 bg-black/50 backdrop-blur-sm">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link
          href="/"
          className="text-xl font-semibold text-zinc-50 transition-colors hover:text-zinc-300"
        >
          FlashyDoc
        </Link>
          ...
          {isSignedIn ? (
            <>
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="relative h-10 w-10 rounded-full border-zinc-700 bg-transparent hover:bg-zinc-800/50"
                  >
            ...
            <>
              <Button
                variant="outline"
                onClick={() => openSignIn()}
                className="border-zinc-700 bg-transparent hover:bg-zinc-800/50 hover:text-white"
              >
                Sign In
              </Button>
              <Button
